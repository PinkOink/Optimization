...
def square(x):
    c = (math.sqrt(2) * x - 1) / (x - 2 * math.sqrt(2) + 2)
    return -(x - (((c - 1) * (c - 1) + x * x / 4) * math.atan(x / (2 * (c - 1))) - x * (c-1) / 2))
...
a, b = 1, 2 * math.sqrt(2)
eps = [0.00001] #[0.1, 0.01, 0.001]
wrp = FuncWrapper(square)
min = []
for e in eps:
    min.append(OneDimMinimizator(wrp, a, b, e))

for m in min:
    ak, bk = m.dichotomy_min()
    print("Answer: (%.5f, %.5f)" % (ak, bk))
    print("Interval abs: %.5f" % (bk - ak))
    print("Func = ", -wrp.get_value((bk + ak) / 2))
    print()
